...
In this exercise, we'll combine two or more data frames using joins. Sometimes, joining adds columns. Other times, it adds rows — this is the case when either or all of the data frames include multiple observations for the same join key.
...
* Columns — we're adding a column that contains the subindustry name.
...
* Yes — it’s often called a many-to-one join, because many firms share the same subindustry code, but each code maps to one subindustry name.
...
* Rows (mainly) — each firm now appears once for each year of (pe) data available.
