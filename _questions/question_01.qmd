...
- Each observation has its own row: each row corresponds to a single firm–year observation (one company in one year), rather than spreading a firm across multiple year-specific columns.

- Each value has its own cell: individual measurements (e.g., a firm’s `price` in a given `fyear`) occupy a single cell, avoiding composite values or encoded metadata.
...
total_sum <- 
  sum(asx_prices_messy$price_2019, na.rm = TRUE) + 
...
# Count the number of non-missing values for each year (2019–2024)
total_count <- 
  sum(!is.na(asx_prices_messy$price_2019)) + 
...
# Compute the overall average price across all years (2019–2024)
overall_average <- 
...
When calculating the average share price across the entire 2019–2024 period, the problem becomes even clearer. With messy wide data, you must explicitly add each `price_YYYY` column, sum them, and divide by the non-missing counts, as in your code. This is inefficient and error-prone, especially as more years are added. If we had data over ten, twenty, or thirty years, this approach would be impractical.
...
```
When we look for the maximum share price in the messy wide data frame over 2023–2024, we can’t just ask “what’s the max of price by year,” because the year is embedded in column names (`price_2023`, `price_2024`). Instead, your solution computes a row-wise maximum across year-columns and then finds the overall maximum across firms.

Conceptually, this requires you to do both: (i) search across columns (take the maximum across `price_2023` and `price_2024` for each row/firm) and then (ii) search down rows (identify the firm with the largest of those row-wise maxima). This two-step scan is a direct consequence of violating the tidy rule “no data in column names”: the time index (year) is encoded in the names of multiple columns rather than existing as values in a single year column. It also conflicts with “each variable has its own column,” because “year” should be a variable, not a naming convention.

As soon as you extend the sample window (e.g., 2019–2024), the approach becomes increasingly inefficient: you now need to expand the pmax() call to include more columns—`price_2019`, `price_2020`, …, `price_2024`—before you can again scan down rows to find the overall maximum. The inefficiency therefore grows primarily with the number of columns (more years → more year-specific price columns to list and manage), not with rows. This column explosion is exactly what tidy data avoids.
...
asx_prices_messy |>
  ...
  theme_minimal()
```
The scatter plot shows a clear positive, nonlinear relationship between earnings per share (EPS) and share price. Firms with higher EPS generally have higher share prices (and the quadratic fit suggests that investors pay more for an extra dollar of earnings per share the higher the level of those earnings per share). This makes sense because a firm’s share price reflects the market’s valuation of its future earnings potential. When investors buy shares, they are essentially buying a claim on the company’s expected future profits. Companies with high EPS signal high EPS into the future, so investors price these firms' shares more highly.

However, if we try to plot this relationship for the entire sample period (2019–2024) using the messy data frame, we run into problems. When data is stored in a messy format, examining relationships between variables—whether through plots, correlations, or regressions—becomes inefficient and error-prone:
...
- Observations fragmented across columns: A single firm–year record is spread across many wide columns. To analyze the relationship over time, you’d need to repeatedly gather the right set of columns (e.g., all `price_*` and `eps_*`) and stitch them back together, rather than treating each firm–year as one row.

- Scaling inefficiency: With more years, the number of columns grows. To study correlations or trends over 2019–2024 (or longer), you would need to search across more and more columns rather than simply extending the dataset down rows. This is the opposite of tidy efficiency, where longer time series just mean more rows.

- Analytical barriers: Many standard R functions (`group_by`, `summarise`, `cor`, regression modeling, plotting) assume tidy inputs—one variable per column and one observation per row. 

**(g). Visualizing messy data**

With messy data—where variable names are embedded in column names we cannot directly map variables to aesthetics in `ggplot` (e.g., `aes(x = year, y = price)`). Instead, each year of share price is stored in a separate column rather than in a single “year” column with associated “price” values. This structure prevents us from easily:

- Creating a consistent x-axis for time (since “year” exists only inside column names rather than as a proper variable).
...
The plot we generated—“Selected Firms’ Indexed Share Price (2019–2024)”—is only possible because we use the data in its tidy form. In this structure:
