...
**(a)**.
Let's start by counting how many firms in the ASX200 belong to each subindustry. Complete the starter code below to count how many firms belong to each subindustry, and return the three most common: 
...
**(b)**. Let's now ask: are there any subindustries that don’t appear in the ASX200 sample?
...
Look thru this output and see if there's any common themes...
...
If all gsubind codes in firm_codes matched a subindustry in the lookup table, then the result will look identical to the `left_join()` output from Exercise 2.
...
`inner_join(x, y)`
→ Keeps only rows that match in both x and y (intersection).
...
Which join only keeps perfect matches?
