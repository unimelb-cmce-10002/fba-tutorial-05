...
```{r}
# Start from firm–subindustry dataset
firms_subindustries |>
...
In Exercise 2(a), when we joined `firms_subind_codes` with `subind_names`, the output only included sub-industries that actually had at least one firm in the ASX 200. This meant sub-industries with no large listed firms simply didn’t appear in the table. The join ignored them because there were no matches on the key (`gsubind`).

`anti_join_df` highlights the underrepresented or absent sub-industries in the ASX 200. These rows tell us: “Here are sectors that exist in the economy (according to GICS), but no top-200 ASX firm belongs to them.”
...
- Firms whose `gsubind` doesn’t match a valid sub-industry code (those appeared in the reversed anti-join).
...
- Construction’s stability is linked to consistent infrastructure and domestic projects (trending upward due to recent, large-scale government projects).
